...
We use **Group Relative Policy Optimization (GRPO)** via Hugging Face TRL and Unsloth (4-bit QLoRA) to train the LLM. 

### The Reward Model (6 Channels)
Instead of a sparse 0/1 win/loss reward, we provide a dense 6-channel reward signal:
...
During our initial GRPO training loop, the model achieved a massive score, but we noticed the standard deviation of the returns across the batch was exactly `0.0`. 
